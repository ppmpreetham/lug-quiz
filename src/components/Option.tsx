const Option = ({option, no, isSelected}:{option: string, no:number, isSelected: boolean}) => {
  return (
    <div className="flex flex-row text-white text-4xl items-center gap-4 w-full px-6 group">
      ...
      <div className={`border-white border h-16 w-full text-center flex items-center justify-center flex-row p-2 cursor-pointer ${isSelected ? 'bg-green-600 text-black border-none' : 'group-hover:bg-white group-hover:text-black group-hover:border-none cursor-pointer'}`}>
        {option}
      </div>
    </div>
  )
